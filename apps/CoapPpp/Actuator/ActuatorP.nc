...
{
  enum {
	QUERY = 126,					// prompt key number from serial port
  };
  ...
  event message_t *UartReceive.receive[am_id_t id](message_t *msg,
						   void *payload,
						   uint8_t len) {	// upon receiving a prompt package from the serial port
	if(msg->data[0] == QUERY)	// if the prompt package is the key number
		post uartSendTask();	// send stored package over the serial port
    return msg;
  }
